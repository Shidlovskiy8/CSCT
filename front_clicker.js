// =================================================================
// ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (Замена API Tampermonkey)
// =================================================================

// Замена GM_setValue на chrome.storage.local
async function setStorageValue(key, value) {
    if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
        ...
        // Fallback на localStorage, если расширение запустилось без прав storage
        localStorage.setItem(`ac_${key}`, typeof value === 'object' ? JSON.stringify(value) : value);
    }
}

// Нативная замена copyToClipboard с помощью Clipboard API
...
if (window.location.hostname.includes('catalogs.avito.ru')) {
    ...
    function injectButton() {
        ...
        const existingBtn = targetEl.parentNode?.querySelector('button[data-autoclick-btn="true"]');
        if (existingBtn) {
            buttonInjected = true;
            return;
        }
        ...
        btn.addEventListener('click', async (e) => {
            ...
            try {
                ...
                // Используем замененную функцию хранения
                await setStorageValue('selected_catalog', catalogText);
                await setStorageValue('extracted_fields', jsonFields);
                await setStorageValue('autoclick_active', true);

                // Оставляем дублирование в localStorage текущего домена (при необходимости)
                try {
...
// =================================================================
// СИСТЕМНОЕ ХРАНИЛИЩЕ И ЛОГИРОВАНИЕ (GM API)
// =================================================================
class StorageManager {
    ...
    static set(key, value) {
        if (typeof GM_setValue === 'function') {
            ...
            try {
                localStorage.setItem(`ac_${key}`, JSON.stringify(value));
            } catch (e) {
    ...
    static logEvent(type, details) {
        const history = this.get('action_history', []);
        history.push({
        ...
        this.set('action_history', history);
    }
}
...
class DebugUI {
    ...
    `;
        ...
        <div style="font-weight: 600; border-bottom: 1px solid #313244; padding-bottom: 8px; display: flex; justify-content: space-between; align-items: center; color: #89b4fa; flex-shrink: 0;">
            ...
            <div style="display: flex; gap: 6px; align-items: center;">
                <button id="ac-ui-pause" title="Пауза / Продолжить" style="background: #45475a; color: #f9e2af; border: none; padding: 2px 6px; border-radius: 4px; cursor: pointer; font-size: 10px;">⏸</button>
    ...
    `;
        ...
        const pauseBtn = document.getElementById('ac-ui-pause');
        ...
        pauseBtn.addEventListener('click', () => {
            ...
            pauseBtn.style.background = this.isPaused ? '#a6e3a1' : '#45475a';
            pauseBtn.style.color = this.isPaused ? '#11111b' : '#f9e2af';
            this.setStatus(this.isPaused ? 'ПАУЗА' : 'РАБОТА', this.isPaused ? '#f38ba8' : '#a6e3a1');
        });
    }
    ...
    log(msg, color = '#bac2de') {
        ...
        StorageManager.logEvent('log', msg);
...
// =================================================================
// ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ И ОБРАБОТЧИКИ ТЕКСТА
// =================================================================
...
// =================================================================
// ОЖИДАНИЕ И ОБРАБОТКА ИНТЕРАКТИВНОСТИ ЭЛЕМЕНТОВ
// =================================================================
function waitForFieldReady(findFn, timeout = 2500) {
    return new Promise((resolve) => {
        ...
        const timer = setInterval(() => {
            if (ui && ui.isPaused) return;

            const blockStatus = checkBlockingElements();
            if (blockStatus === 'CAPTCHA_DETECTED') {
                if (ui) ui.setStatus('ТРЕБУЕТСЯ КАПЧА', '#ff5555');
            }
...
// =================================================================
// МОДУЛЬ ЗАГРУЗКИ ФАЙЛОВ И ИЗОБРАЖЕНИЙ
// =================================================================
async function uploadFileToInput(fileInputSelector, fileUrl, fileName = 'image.jpg') {
    try {
        const input = await waitForFieldReady(() => document.querySelector(fileInputSelector), 3000);
        ...
        if (ui) ui.log(`📸 Файл "${fileName}" успешно прикреплен`, '#50fa7b');
        return true;
    } catch (e) {
        if (ui) ui.log(`❌ Ошибка загрузки файла: ${e.message}`, '#ff5555');
        return false;
    }
}

// =================================================================
// КЕШИРОВАНИЕ И СЕТЕВЫЕ ЗАПРОСЫ
// =================================================================
