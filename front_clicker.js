// ==UserScript==
// @name         Avito Front AutoClicker (Pure JS)
// @namespace    http://tampermonkey.net/
// @version      8.60
// @description  Автокликер фронта Avito на чистом JavaScript
// @author       VacaVaca
// @match        https://catalogs.avito.ru/*
    ...
    init() {
        ...
        this.container.innerHTML = `
            <div style="font-weight: 600; border-bottom: 1px solid #313244; padding-bottom: 8px; display: flex; justify-content: space-between; align-items: center; color: #89b4fa; flex-shrink: 0;">
                <span>⚡ Автокликер <span style="font-size: 9px; opacity: 0.7;">v8.60 (JS)</span></span>
...
// =================================================================
// РАБОТА С СЕТЬЮ И КЕШИРОВАНИЕМ ДАННЫХ
// =================================================================
...
async function promptUserForCustomValue(fieldName, options) {
    return new Promise((resolve) => {
        ...
        `;
        modal.innerHTML = `
            <div style="font-weight: bold; margin-bottom: 12px; font-size: 14px; color: #50fa7b;">Выберите значение для "${fieldName}":</div>
            <select id="ac-custom-select" style="width: 100%; padding: 8px; background: #44475a; color: #fff; border: 1px solid #6272a4; border-radius: 4px; margin-bottom: 15px; outline: none; font-size: 14px;">
                ${options.map(opt => `<option value="${opt}">${opt}</option>`).join('')}
            </select>
            <button id="ac-custom-btn" style="width: 100%; padding: 8px; background: #50fa7b; color: #282a36; font-weight: bold; border: none; border-radius: 4px; cursor: pointer;">Подтвердить</button>
        `;
        document.body.appendChild(modal);

        document.getElementById('ac-custom-btn').onclick = () => {
            const val = document.getElementById('ac-custom-select').value;
    ...
    function injectButton() {
        ...
        btn.addEventListener('click', async (e) => {
            ...
            try {
                ...
                // Сохраняем во внутреннее хранилище скрипта вместо chrome.storage
                GM_setValue('autoclick_active', true);
                GM_setValue('selected_catalog', catalogText);
                GM_setValue('extracted_fields', JSON.stringify(dynamicValues));
...
if (window.location.hostname.includes('avito.ru') && window.location.pathname.includes('/additem')) {
    if (GM_getValue('autoclick_active', false)) {
        ScrollLock.lock();
        GM_setValue('autoclick_active', false);

        const selectedCatalog = GM_getValue('selected_catalog', '');
        let extractedFields = [];
        try { extractedFields = JSON.parse(GM_getValue('extracted_fields', '[]')); } catch(e) {}
