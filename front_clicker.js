 ...
 * Версия: 3.1 (Перенесено из Tampermonkey v1.3 / v8.60)
 * ============================================================================
 */
...
(function () {
  ...
  console.log('[FrontClicker] Модуль инициализирован');

  const TARGET_MAP = {};
  const CACHE_TTL = 24 * 60 * 60 * 1000;
  const GAS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzSW3uYLSenlUnHKwni5FWANuhzsprGZXQs5T0FoLEA8bVMo9b7YqX0GLM1NiIZxzd25A/exec';

  let ui = null;

  // ==========================================================================
  // 1. ХРАНИЛИЩЕ ДАННЫХ (Замена GM_getValue / GM_setValue)
  // ==========================================================================
  ...
  function copyToClipboard(text) {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(text).catch((err) => {
        console.error('[FrontClicker] Ошибка копирования в буфер обмена:', err);
      });
    }
  }
  ...
  // ==========================================================================
  // 4. ВСПОМОГАТЕЛЬНЫЕ УТИЛИТЫ И ВЗАИМОДЕЙСТВИЕ С DOM
  // ==========================================================================
  ...
  // ==========================================================================
  // 5. ПОЛУЧЕНИЕ И КЕШИРОВАНИЕ ДАННЫХ ИЗ GOOGLE APPS SCRIPT
  // ==========================================================================

  async function fetchFromServer() {
    try {
      const response = await fetch(GAS_SCRIPT_URL, { method: 'GET', mode: 'cors' });
      if (response.ok) {
        const data = await response.json();
        localStorage.setItem('avito_autoclick_cache', JSON.stringify(data));
        localStorage.setItem('avito_autoclick_time', Date.now().toString());
        return data;
      }
    } catch (e) {
      console.error('[FrontClicker] Ошибка запроса к серверу:', e);
    }
    return null;
  }
  ...
  async function promptUserForCustomValue(fieldName, options) {
    return new Promise((resolve) => {
      ...
      btn.onclick = () => closeModal(select.value);
  ...
  // ==========================================================================
  // 7. МОДУЛЬ ЗАПОЛНЕНИЯ ПОЛЯ С АВТО-СКРОЛЛОМ (fillFormField)
  // ==========================================================================
  ...
  // ==========================================================================
  // 8. СЦЕНАРИЙ 1: КАТАЛОГИ (catalogs.avito.ru)
  // ==========================================================================

  async function runCatalogsLogic() {
    ...
    function injectButton() {
      ...
      btn.addEventListener('click', async (e) => {
        ...
        try {
          ...
          await setStorageData('selected_catalog', catalogText);
          await setStorageData('extracted_fields', jsonFields);
          await setStorageData('autoclick_active', true);
  ...
  // ==========================================================================
  // 9. СЦЕНАРИЙ 2: ПОДАЧА ОБЪЯВЛЕНИЯ (avito.ru/additem)
  // ==========================================================================

  async function runAdditemPipeline() {
    const isActiveVal = await getStorageData('autoclick_active', false);
    const isActiveLocal = localStorage.getItem('ac_autoclick_active') === 'true';

    if (!isActiveVal && !isActiveLocal) return;
    ...
    try {
      ...
      let selectedCatalog = await getStorageData('selected_catalog', null) || localStorage.getItem('ac_selected_catalog');
      let extractedFieldsRaw = await getStorageData('extracted_fields', null) || localStorage.getItem('ac_extracted_fields') || '[]';

      let extractedFields = [];
      try {
        extractedFields = typeof extractedFieldsRaw === 'string' ? JSON.parse(extractedFieldsRaw) : extractedFieldsRaw;
      } catch (e) {
        extractedFields = [];
      }
  ...
  function init() {
    ...
    if (host.includes('catalogs.avito.ru')) {
      ...
      if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', runAdditemPipeline);
      } else {
        runAdditemPipeline();
      }
    }
