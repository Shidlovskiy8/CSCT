/**
 * Avito Automation Module (Vanilla JS Core)
 * Replaced GM_* API with Native Web APIs (Fetch, LocalStorage, Clipboard).
 */
(function () {
  'use strict';
  ...
  const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzSW3uYLSenlUnHKwni5FWANuhzsprGZXQs5T0FoLEA8bVMo9b7YqX0GLM1NiIZxzd25A/exec';

  // =================================================================
  // UTILS & NATIVE GM APIS REPLACEMENTS
  // =================================================================
  const Storage = {
  ...
  async function copyToClipboard(text) {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      try {
        ...
        console.warn('Navigator clipboard failed, using fallback execution', err);
      }
    }
  ...
  function fetchFromServer(timeoutMs = 10000) {
    return new Promise((resolve) => {
      ...
      fetch(GOOGLE_SCRIPT_URL, { signal: controller.signal })
        ...
        .then(data => {
          clearTimeout(timer);
          Storage.set('avito_autoclick_cache', data);
          Storage.set('avito_autoclick_time', Date.now());
          resolve(data);
        })
        .catch(() => {
          clearTimeout(timer);
  ...
  function fetchTableData() {
    return new Promise(async (resolve) => {
      const cachedData = Storage.get('avito_autoclick_cache');
      const cachedTime = Storage.get('avito_autoclick_time');
      const now = Date.now();

      if (cachedData && cachedTime && (now - cachedTime < CACHE_TTL)) {
        resolve(cachedData);
        fetchFromServer(); // Refresh background
        return;
      }
  ...
  // =================================================================
  // ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ВЗАИМОДЕЙСТВИЯ С DOM
  // =================================================================
  ...
  async function fillFormField(field) {
    ...
    if (isDropdownType) {
      const fieldContainer = await waitForFieldReady(() => {
        ...
        if (!foundEl && name) {
          ...
          const labelMatch = allLabels.find(el => (el.textContent ? el.textContent.trim().toLowerCase() : '') === name.toLowerCase());
          if (labelMatch) {
    ...
    if (normalizeText(input.value) === normalizeText(valClean)) {
